...
import { getAutores } from "@/utils/repo_tesis/autores/getCatalogoAutores";
import { AutorType } from "@/models/autor";
import { CoordinacionCatalogoType, CoordinacionType } from "@/models/coordinacion";
import { getCoordinaciones } from "@/utils/coordinaciones/getCoordinaciones";
...
const ListaTesis = ({
  ...
  return (
    <div className={className}>
      ...
      <button onClick={() => downloadCSVTesisMaestro({data: currentTesisMini,token, columnsToInclude:[true, true, true, true, true, true, true], catalogoRolesTesis, catalogoPronaces: pronaceList })}>Download CSV</button>
      {showGraph && (
...
const downloadCSVTesisMaestro = async ({
  data,
  token,
  columnsToInclude,
  catalogoRolesTesis,
  ...
  data: TesisMiniMaestroType[],
  token: string,
  columnsToInclude: [boolean, boolean, boolean, boolean, boolean, boolean, boolean],
  catalogoRolesTesis: RolTesisType[],
  ...
  let catalogoAutores: AutorType[] = [];
  if(columnsToInclude[3]){
    const response = await getAutores(token);
    if(response.success){
      catalogoAutores = response.autores;
    }
  }
  let catalogoCoordinaciones: CoordinacionCatalogoType[] = [];
  if(columnsToInclude[6]){
    const response = await getCatalogoCoordinaciones(token);
    if(response.success){
      catalogoCoordinaciones = response.catalogo_coordinaciones;
    }
  }

  console.log('Catalogo Autores:', catalogoAutores);
  console.log('Catalogo Coordinaciones:', catalogoCoordinaciones);


  // Define functions to create entries for each column
  const getId = (row) => {
    return row.id || '';
  };
  ...
  const getRolComite = (row) => {
    const rol = catalogoRolesTesis.find((r) => r.id === row.id_rol_comite)?.rol_tesis || '';
    return rol;
  };
  const getAutor = (row) => {
    const autor = catalogoAutores.find((a) => a.id === row.id_autor)?.autor || '';
    return autor;
  };
  const getAno = (row) => {
    return row.fecha ? new Date(row.fecha).getFullYear() : '';
  };
  const getPronace = (row) => {
    const pronace = catalogoPronaces.find((p) => p.id === row.id_pronace)?.pronace || '';
    return pronace;
  }; // Replace with correct property if different
  const getCoordinacion = (row) => {
    const coordinacion = catalogoCoordinaciones.find((c) => c.id === row.id_coordinacion)?.coordinacion || '';
    return coordinacion;
  }; // Replace with correct property if different



  // Define header names and corresponding functions
  const headers = ['id', 'titulo', 'rol_comite', 'autor', 'año', 'pronace', 'coordinacion'];
  const functions = [getId, getTitulo, getRolComite, getAutor, getAno, getPronace, getCoordinacion];

  // Filter headers and functions based on columnsToInclude array
  const filteredHeaders = headers.filter((_, index) => columnsToInclude[index]);
  const filteredFunctions = functions.filter((_, index) => columnsToInclude[index]);

  // Create rows for CSV based on filtered functions
  const csvRows = data.map((row) =>
    filteredFunctions.map((fn) => `"${fn(row)}"`).join(",")
  );

  // Create CSV content
  const csvContent = [filteredHeaders.join(","), ...csvRows].join("\n");
  ...
  link.setAttribute("download", "currentTesisMini.csv");
